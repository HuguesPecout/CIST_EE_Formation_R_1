...
**Enregistrement vidéo de la séance disponible à ce lien** : *A venir*
</div>
...
**5.** Créez un nouveau fichier de script R.
  ...
  - Ou Raccourci clavier :`ctrl`+`SHIFT`+`N` puis `ctrl`+`s` pour l'enregistrer
...
**7.** Téléchargez le fichier "*wb_don_1990_2019.xls*" mis à disposition sur le *Sharedocs* (https://bit.ly/3683nRN). Ce fichier comporte des  données sont issues de la Banque Mondiale. Enregistrez-le dans le sous-répertoire *data*. Cette opération peut être réalisée "manuellement", mais peut également être automatisée avec R : 

```{r , eval=FALSE, echo=TRUE}
download.file(url = "https://sharedocs.huma-num.fr/wl/?id=OIdQC31yRfOfrXLPqCfWCxhNffoVrNXj&mode=grid&download=1",
              destfile = "data/wb_don_1990_20192.xlsx")
```
...
**10.** Utilisez la fonction `read_excel()` pour importer le 3e onglet du fichier de données "*wb_don_1990_2019.xls*". **Assignez-le dans un objet**.

```{r, eval=FALSE, echo=TRUE}
# Accédez à l'aide de la fonction read_excel()
?read_xls

# L'argument sheet vous permet de choisir quel onglet à importer, ex :
...
**11.** Explorez votre nouvel objet créé, qui contient le tableau de données importé :
   ...
   - En clic-bouton avec l'interface Rstudio (dans la fenêtre situé en haut à droite, onglet *Environment*)
   - En appliquant les fonctions suivantes sur votre objet:
   

```{r, eval=FALSE, echo=TRUE}

# Ouvrir le tableau de l'IDE Rstudio :
View()
...
**12.** Sélectionner uniquement les pays de la région Sub-saharienne ET les colonnes de 1 à 13 :
...
# OU Sélection avec fonction primitive subset()
subset(x = ......, 
       subset =  region %in% "......", 
       select = .... )
...
**13.** Calculer la population totale (variable *POP*) des pays Sub-Sahariens :
...
**14.** Afficher un nuage de point avec la fonction `plot()` où :
...
<div class="alert alert-info" role="alert">
**Un projet Rstudio avec la correction de l'exercice est disponible en téléchargement à** [**ce lien**](https://github.com/HuguesPecout/CIST_EE_Formation_R_1/archive/refs/heads/master.zip).
</div>
